#代码修改过，该代码适用于cp2k2023版本，20230719
# %%
import math
import re
...
delimiter_patterns = []
delimiter_p1 = re.compile(r"^ MD_INI\| MD initialization")
delimiter_p2 = re.compile(r"^ MD_INI\| Cell angles")
delimiter_p3 = re.compile(r"^ MD\| \*+")
delimiter_patterns.append(delimiter_p1)
delimiter_patterns.append(delimiter_p2)
delimiter_patterns.append(delimiter_p3)
avail_patterns = []
avail_patterns.append(re.compile(r"^ MD_INI\| Potential energy"))
avail_patterns.append(re.compile(r"^ MD\| Step number"))


class Cp2kSystems:
    """deal with cp2k2023 outputfile."""
    ...
    def handle_single_log_frame(self, lines):
        ...
        energy_pattern_1 = re.compile(
            r" MD_INI\| Potential energy \[hartree\]\s+(?P<number>\S+)"
        )
        #  CONSERVED QUANTITY [hartree] =                              -0.279168013085E+04
        energy_pattern_2 = re.compile(
            r" MD\| Potential energy \[hartree\]\s+(?P<number>\S+)"
        )
        energy = None
        cell_length_pattern = re.compile(
            r" (MD_INI\| ){0,1}Cell lengths \[bohr\]\s+(?P<A>\S+)\s+(?P<B>\S+)\s+(?P<C>\S+)"
        )
        cell_angle_pattern = re.compile(
            r" (MD_INI\| ){0,1}Cell angles \[deg\]\s+(?P<alpha>\S+)\s+(?P<beta>\S+)\s+(?P<gamma>\S+)"
        )
        ...
        for line in lines:
            ...
            if stress_sign in line:
                stress_flag = 1
        """
        stress_sign = " STRESS| Analytical stress tensor"
        stress_end_sign = " STRESS| 1/3 Trace"
        stress_flag = 0
        stress = []

        for line in lines:
            if stress_flag == 2:
                if stress_end_sign in line:
                    stress_flag = 0
                else:
                    stress.append(line.split()[2:5])
            if stress_flag == 1:
                stress_flag = 2
            if stress_sign in line:
                stress_flag = 1
