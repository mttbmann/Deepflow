...
class UtilizationTools:
    ...
    @staticmethod
    def avg_bond_length(iteration=all, min_dist_thresh=1.0, max_dist_thresh=1.85, stride=1):
        """
        ...
        if iteration==all:
            ...
            traj = [sorted(glob.glob('**/iter.*/01.model_devi/**/all.lammpstrj'))[-1]]
            timesteps, pbc_length, time_indices = UtilizationTools.read_trajectory(traj)
        else:
            traj = [sorted(glob.glob('**/iter.*/01.model_devi/**/all.lammpstrj'))[iteration]]
            timesteps, pbc_length, time_indices = UtilizationTools.read_trajectory(traj)
